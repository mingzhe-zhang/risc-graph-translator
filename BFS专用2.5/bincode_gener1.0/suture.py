...
def get_wait_result_code():
    code = ('#---------------------------------------\n',
            ...
            '\n')
    return ''.join(code)
def get_new_data_load(PE_num, tiling, height2, exeblock_per_pe, vector_addr_start, data_list, col_tiling):
    start = 58880
    code = ''
...
def main():
    ...
    vector_addr_start = '58880'     #后续新向量在sram中暂时存储的地址
    level_addr_start = '59392'
    warning = ''
    filelist = file_name(os.path.dirname(os.path.abspath(__file__))+'\\test\\')
    pe_code_start_pos = '00001500'
    ...
    tiling = int(width / (PE_num / col_tiling[0] * exeblock_per_pe))  #每个exeblock要运算多少行
    ...
    riscv_code += get_new_data_load(PE_num, tiling, height2, exeblock_per_pe, int(data_start_pos), data_list, col_tiling)
