...
def translator(filename, isa_def_filename='risc-graph-isa-def.xml', inst_table_filename='risc-graph-inst-table.xml'):
...
#        main(sys.argv[1])   #需要从命令行输入文件名
    ...
    if(options.in_filename!=None and options.out_filename!=None):
        ...
        main('insts.txt', 'binary_inst.txt')    #默认输入输出文件
