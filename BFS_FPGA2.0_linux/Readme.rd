...
BFS_FPGA2.0:
提供了graph500的接口，在json中的相关位置输入图文件地址即可，由于脚本设置的起始顶点是第一个，所以有可能会出现全0的情况，请增大边的数量
多试几次，并且建议最多只使用8个pe，因为本脚本不支持处理负载不均衡的情况。

BFS_FPGA2.0_linux:
添加了对linux系统的支持，需要为BFS_FPGA2.0_linux\bincode_gener1.0\jupiter\bin\文件夹中的java以及jupiter文件赋予可执行权限(chmod +x)，
首先使用graph500生成图，之后在pe_module_input.json中的第一个参数输入相应路径(注意使用"/")，之后运行data_load.py，等待载入结束，执行
Runthis.py

注: 使用python3.5以下版本时按照报错信息把相应位置的指定函数类型代码删掉即可