...
vertex_num = width2+1    #这个是总的vertex数量+1
starting_addr = 3072     #这个最终读SRAM结果的起始地址
...
########################################################################################################
    ...
    loop_str += '\tinitial begin\n'
    for i in range(tile_num):    #可变部分循环
        loop_str += '\t\t$readmemh("./case/<replace>/TILE/tile' + str(i) + '/dram_data.txt", dram_data_file_'+ str(i+1) + ');\n'
        loop_str += '\t\t$readmemh("./case/<replace>/TILE/tile' + str(i) + '/dram_data_addr.txt", dram_addr_file_'+ str(i+1) + ');\n'
        loop_str += '\t\t$readmemh("./case/<replace>/TILE/tile' + str(i) + '/sram_code.txt", sram_data_file_'+ str(i+1) + ');\n'
        loop_str += '\t\t$readmemh("./case/<replace>/TILE/tile' + str(i) + '/sram_code_addr.txt", sram_addr_file_'+ str(i+1) + ');\n'
    ...
    loop_str += '//-------------------------------------------------------------\n'
    loop_str += '\tlocalparam [20:0]\n'
    loop_str += '\t\tS_IDLE 		        = 21\'d0,\n'
    loop_str += '\t\tS_cfg                  = 21\'d1,\n'
    loop_str += '\t\tS_cfg_polling          = 21\'d2,\n'
    for i in range(tile_num):    #可变部分循环
        loop_str += '\t\tS_transfer_'+ str(i+1) + '        =21\'d' + str(2*i+3) + ',\n'
        loop_str += '\t\tS_polling_'+ str(i+1) + '         =21\'d' + str(2*i+4) + ',\n'
    loop_str += '\t\tS_reading_results          = 21\'d' + str(2*tile_num+3) + ',\n'
    loop_str += '\t\tS_end                      = 21\'d' + str(2*tile_num+4) + ';\n'
    loop_str += '\treg [20 : 0] S_state;\n'     
    locv.writelines((loop_str))
########################################################################################################
    # fixed content 2
...
########################################################################################################
    ...
    for i in range(tile_num):    #可变部分循环
        ...
        if i+1 != tile_num :
            ...
            loop_str += '\t\t\t\tend\n'
            loop_str += '\t\t\t\tR_ok 				<= CENA;\n'
            loop_str += '\t\t\t\tR_ok_1 			<= R_ok;\n'
            loop_str += '\t\t\t\tsram_write_num_'+ str(i+1) + '  	<= 0;\n'
            loop_str += '\t\t\tend\n'
        else :
            loop_str += '\t\t\telse if ( S_state == S_polling_'+ str(i+1) + ' ) begin\n'
            loop_str += '\t\t\t\tif (R_ok_1 && QA == 32\'hf) begin\n'
            loop_str += '\t\t\t\t\tCENA			<= 1\'b0;\n'
            loop_str += '\t\t\t\t\ttask_end_'+ str(i+1) + '		<= 1;\n'
            loop_str += '\t\t\t\tend\n'
            loop_str += '\t\t\t\telse if (R_ok_1 && QA != 0) begin\n'
            loop_str += '\t\t\t\t\tCENA			<= 1\'b0;\n'
    ...
    loop_str += '\t\t\telse if ( S_state == S_reading_results ) begin\n'
    loop_str += '\t\t\t\tif (results_addr == '+ str(vertex_num) +') begin\n'
    loop_str += '\t\t\t\t\tCENA			<= 1\'b0;\n'
    loop_str += '\t\t\t\t\treading_end		<= 1;\n'
    loop_str += '\t\t\t\tend\n'
    loop_str += '\t\t\t\telse if (reading_end == 1) begin\n'
    loop_str += '\t\t\t\t\tCENA			<= 1\'b0;\n'
    loop_str += '\t\t\t\tend\n'
    loop_str += '\t\t\t\telse begin\n'
    loop_str += '\t\t\t\t\tCENA			<= 1\'b1;\n'
    loop_str += '\t\t\t\t\tAA				<= '+ str(starting_addr) +' + results_addr;\n'
    locv.writelines((loop_str))
########################################################################################################
    # fixed content 7
