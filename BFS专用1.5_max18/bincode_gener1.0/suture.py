...
def get_result_init_code(ST_addr_start, ST_addr_end):
    code = ('#---------------------------------------\n',
            '# 结果初始化：\n',
            '# a1: 结果dram末地址\n',
            '# t1: 结果dram首地址\n',
            ...
            '    li a1, '+hex(int(ST_addr_end/4+1)*4)+'     # 末地址\n',
            'result_init_loop:\n',
...
def get_wait_result_code(ST_addr_start, ST_addr_end, vector_addr_start, width, PE_num, tiling, height2, exeblock_per_pe, data_list, Label_no):
    code = ('#---------------------------------------\n',
            ...
            '    beq s0, a2, wait_loop'+Label_no+' \n',
            '    addi a3, a3, -1\n',
            '    beqz a3, buffer0_'+Label_no+'    \n',
            '    addi a3, a3, 1\n',
            
            '    beq s1, a2, wait_loop'+Label_no+' \n',
            '    addi a3, a3, -2\n',
            '    beqz a3, buffer1_'+Label_no+'    \n',
            '    addi a3, a3, 2\n',
            
            '    beq s2, a2, wait_loop'+Label_no+' \n',
            '    addi a3, a3, -3\n',
            '    beqz a3, buffer2_'+Label_no+'    \n',
            '    addi a3, a3, 3\n',
            
            '    beq s3, a2, wait_loop'+Label_no+' \n',
            '    addi a3, a3, -4\n',
...
def get_switch2riscv_code(ST_exeblock_no, PE_num, exeblock_per_pe, Label_no):
    flag = 0
    for i in range(PE_num):
...
def main():
    ...
    vector_addr_start = '30208'     #后续新向量在sram中暂时存储的地址
    warning = ''
    filelist = file_name(os.path.dirname(os.path.abspath(__file__))+'\\test\\')
    pe_code_start_pos = '00002000'
    ...
    riscv_code += get_result_init_code(ST_addr_start, ST_addr_end)
    riscv_code += '\n'
    ...
    riscv_code += get_switch2riscv_code(PE_num*exeblock_per_pe-1, PE_num, exeblock_per_pe, '1')
    ...
    for x in config_copy_list:
        riscv_code += ''.join(x)
    riscv_code += get_switch2riscv_code(PE_num*exeblock_per_pe-1, PE_num, exeblock_per_pe, '2')
