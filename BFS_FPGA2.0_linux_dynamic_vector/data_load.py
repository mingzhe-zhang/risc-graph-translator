...
for i in range(len(op)-1):        
    input_file_name = res[str(i+1)]['input_file_name']
    ...
    with open(os.path.dirname(__file__) +input_file_name, "r") as d:
        data = d.readlines()
    #print(data)
    ...
    for j in range(m2_height):
        ...
        if int((j+1)/m2_height*100) in print_list:
            print("已完成 "+str(int((j+1)/m2_height*100))+'%')
            print_list.remove(int((j+1)/m2_height*100))
