...
#     print('\n')
#     output += '\n'
# print(os.path.dirname(os.path.abspath(__file__))+ '/vector_input/vec_ind_example.txt')
with open(os.path.dirname(os.path.abspath(__file__))+ '/vector_input/vec_ind_example.txt', "r") as vec_file:
    vec_ind = vec_file.readlines()

with open(os.path.dirname(os.path.abspath(__file__))+ '/vector_input/iter_limit_example.txt', "r") as iter_limit_file:
    iter_limit = iter_limit_file.readlines()

vec_ind = [int(v.rstrip('\n')) for v in vec_ind]

vector_a = list()
for i in range(m1_width):
    vector_a.append(0)
    ...
    for j in range(reload_num*reload_num):
        for k in range(int(PE_num/col_tiling[0])):
            for m in range(int(PE_num/col_tiling[0])):
                temp_temp = temp
                for h in range(reload_num-1):
                    temp_temp = np.array(temp_temp) | np.array(temp[int(j/reload_num)*int(PE_num/col_tiling[0])+int(PE_num/col_tiling[0])*reload_num*(h)+m]) | np.array(temp[int(j/reload_num)*int(PE_num/col_tiling[0])+int(PE_num/col_tiling[0])*reload_num*(h+1)+m])
                    #print(int(j/reload_num)*int(PE_num/col_tiling[0])+int(PE_num/col_tiling[0])*reload_num*(h)+m)
                tile_m1[j][k][m*col_tiling[1] : m*col_tiling[1]+col_tiling[1]] = np.array(tile_m1[j][k][m*col_tiling[1] : m*col_tiling[1]+col_tiling[1]]) | np.array(temp_temp[int(j/reload_num)*int(PE_num/col_tiling[0])+m])
