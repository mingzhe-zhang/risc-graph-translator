...
def Exeblock_init(assem, exeblock_no, PE_no, exeblock_per_PE, PE_num, data, data_num):
    ...
    Inst_DRAM_Address = int(256 * 1024 *1024 / 8 / exeblock_per_PE / PE_num / 4 * (exeblock_no + PE_no * exeblock_per_PE ) / 4 + 1) * 4      #每一个exeblock的代码段分配分配给每个exeblock的一半
    ...
    LD_Base, ST_Base = int(256 * 1024 *1024 / 8 / exeblock_per_PE / PE_num / 4 * (0 + PE_no * exeblock_per_PE) / 4  + 1) * 4 + 1024, 0    #数据段基地址在代码段之后，每个exeblock分配分配给每个exeblock的一半，ST地址默认从0开始
