...
def CAL_stage(matrix1, matrix2):
    ...
    for i in range(len(matrix2[0])):
        for j in range(len(matrix1[0])):
            if int(matrix1[0][j][2]/256)==int(matrix2[i][len(matrix1[0])-j-1][2]/256):
        ...
        for j in range(len(matrix1[0])-1):
            if int(matrix2[i][j][2]/256)==int(matrix2[i][j+1][2]/256):
                c = 'PREREAD0 ' + str(matrix2[i][j][2]) +', 0, 0' 
                CAL_stage_assem.append(c)
...
def Exeblock_init(assem, exeblock_no, PE_no, exeblock_per_PE):
    ...
    ST_Base, LD_Base = 0, 1     #数据段高10位偏移为1
    PE = PE_no
    Sparse = 0
    R_N = 1     
    ...
    if(xml_inf['R_N']!=None):
        b_inf.insert(int(xml_inf['R_N']['start']), opr_ext(xml_inf['R_N']['width'], str(R_N)) )
...
def main(height1, width1, height2, width2, PE_num, exeblock_per_PE):
    ...
    for i in range(PE_num):
        ...
        for j in range(exeblock_per_PE):
            ...
            a = matrix_gen(height1, width1, m, (height1*width1+height2*width2)*(i*exeblock_per_PE+j))
            b = matrix_gen(height2, width2, height1*width1+512+n, (height1*width1+height2*width2)*(i*exeblock_per_PE+j)+(height1*width1))
    ...
    for i in range(PE_num):     
        for j in range(exeblock_per_PE):
            Exeblock_init(assem, i*exeblock_per_PE + j, i, exeblock_per_PE)           
